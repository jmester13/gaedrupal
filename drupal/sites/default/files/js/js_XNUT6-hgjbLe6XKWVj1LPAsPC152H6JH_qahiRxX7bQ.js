...
function generate() {
    html2canvas(jQuery('#block-gae-theme-content')[0]).then((canvas) => {
        ...
        saveAs(uri + 'jspdf.jpg');
        // }
      });

    window.jsPDF = window.jspdf.jsPDF;
    window.html2canvas = html2canvas;
    jQuery('.layout-content').attr("data-html2canvas-ignore", "true"); //setter
    var doc = new jsPDF('l', 'mm');
    //doc.autoTable({ html: '#block-gae-theme-content' }); // Know this works.
    var elem = document.getElementById('block-gae-theme-content');
    doc.html(elem);
    doc.save( "jsPDF.pdf");
    location.reload();
  }
